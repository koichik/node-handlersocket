...
vows.describe('Utilities').addBatch({
  ...
  'encodeRequest': {
    'with basic string': {
      topic: function() {
        return hs._encodeRequest(
          ['P', '1', 'database', 'table', 'PRIMARY', 'aaa,bbb,ccc']);
      },
      'should return tab separated strings': function(request) {
        assert.equal(request, 'P\t1\tdatabase\ttable\tPRIMARY\taaa,bbb,ccc\n');
      }
    },

    'indludes control character': {
      topic: function() {
        return hs._encodeRequest(['1', '=', '2', 'a\ta', 'b\nb', 1, 0]);
      },
      'should return tab separated strings with encoded fields':
          function(request) {
    ...
    'includes empty fields': {
      topic: function() {
        return hs._encodeRequest(['1', '=', '2', '', '', 1, 0]);
      },
      'should return tab separated string with empty fields':
          function(request) {
        assert.equal(request, '1\t=\t2\t\t\t1\t0\n');
      }
    },

    'includes null fields': {
      topic: function() {
        return hs._encodeRequest(['1', '=', '2', null, null, 1, 0]);
      },
      'should return tab separated string with nul fields': function(request) {
        assert.equal(request, '1\t=\t2\t\u0000\t\u0000\t1\t0\n');
      }
    }
  },
  ...
  'handleResponse': {
    'gets sucess': {
      topic: function() {
        hs._handleResponse('0\t1\t\t\u0000\n', this.callback);
      },
      'should pass a null to error': function(err, response) {
        assert.isNull(err);
      },
      'shoud pass an array which contains 1 record with 2 fields':
    ...
    'gets error with message': {
      topic: function() {
        return hs._handleResponse('1\t1\thoge\n', this.callback);
      },
      'should pass an Error object to error': function(err, response) {
        assert.instanceOf(err, Error);
      },
