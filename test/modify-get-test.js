...
var suite = vows.describe('Modify and Get')
suite.addBatch({
...
suite.addBatch({
  'updating' : {
    topic : function() {
      ...
      openIndex({ port : 9999, auth : 'node' }, function(err, index) {
        if (err) return self.callback(err);
        index.updateAndGet('=', 100, [ 100, '8888', 'KOICHIK2', 2 ],
                           self.callback);
      })
    },
    'should update one row' : function(err, results) {
      assert.isNull(err);
      assert.lengthOf(results, 1);
      assert.deepEqual(results[0], [ '100', '9999', 'KOICHIK', '1' ]);
    },
...
suite.addBatch({
  'updating with IN' : {
    ...
    'should update two rows' : function(err, results) {
      assert.isNull(err);
      assert.lengthOf(results, 2);
      assert.deepEqual(results[0], [ 'KOICHIK2', '2' ]);
      assert.deepEqual(results[1], [ 'EBIYURI', '1' ]);
    },
    teardown : function() {
      con.close();
    }
...
suite.addBatch({
  'updating with filter' : {
    ...
    'should update one row' : function(err, results) {
      assert.isNull(err);
      assert.lengthOf(results, 1);
      assert.deepEqual(results[0], [ 'KOICHIK', '3' ]);
    },
    teardown : function() {
      con.close();
    }
  ...
  'increment' : {
    topic : function() {
      ...
      openIndexVersion({ port : 9999, auth : 'node' }, function(err, index) {
        if (err) return self.callback(err);
        index.incrementAndGet('=', 100, 1, self.callback);
      })
    },
    'should update one row' : function(err, results) {
      assert.isNull(err);
      assert.lengthOf(results, 1);
      assert.deepEqual(results[0], [ '3' ]);
    },
    teardown : function() {
      con.close();
    }
...
suite.addBatch({
  'decrement' : {
    topic : function() {
      ...
      openIndexVersion({ port : 9999, auth : 'node' }, function(err, index) {
        if (err) return self.callback(err);
        index.decrementAndGet('=', 101, 1, self.callback);
      })
    },
    'should update one row' : function(err, results) {
      assert.isNull(err);
      assert.lengthOf(results, 1);
      assert.deepEqual(results[0], [ '4' ]);
    },
    teardown : function() {
      con.close();
    }
...
suite.addBatch({
  'deleting' : {
    topic : function() {
      ...
      openIndex({ port : 9999, auth : 'node' }, function(err, index) {
        if (err) return self.callback(err);
        index.deleteAndGet('>=', 100, { limit: 10 }, self.callback);
      })
    },
    'should delete two rows' : function(err, results) {
      assert.isNull(err);
      assert.lengthOf(results, 2);
      assert.deepEqual(results[0], [ '100', '8888', 'KOICHIK', '4' ]);
      assert.deepEqual(results[1], [ '101', '9998', 'EBIYURI', '3' ]);
